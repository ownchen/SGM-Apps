...
(function (b, f) {
    ...
    b.jPlayer.prototype = {
        ...
        _lrcLoad: function (lrc, fun) {
            if (!lrc) {
                $(this.options.lrcid).html("<li>No lyrics</li>");
                return;
            };

            if (lrc.indexOf("http") != -1 || lrc.indexOf(".lrc") != -1) {
                $.ajax({
                    url: lrc,
            ...
            if (sname) sname["time"] ? this._addLrc(this.options.lrcClassV1, sname["name"], sname["time"]) : this._addLrc(this.options.lrcClassV1, sname);
            if (cname) cname["time"] ? this._addLrc(this.options.lrcClassV1, cname["name"], cname["time"]) : this._addLrc(this.options.lrcClassV1, cname);
            if (qname) qname["time"] ? this._addLrc(this.options.lrcClassV1, qname["name"], qname["time"]) : this._addLrc(this.options.lrcClassV1, qname);
            if (bname) bname["time"] ? this._addLrc(this.options.lrcClassV1, bname["name"], bname["time"]) : this._addLrc(this.options.lrcClassV1, bname);
            if (sgname) sgname["time"] ? this._addLrc(this.options.lrcClassV1, sgname["name"], sgname["time"]) : this._addLrc(this.options.lrcClassV1, sgname);
            if (special) special["time"] ? this._addLrc(this.options.lrcClassV1, special["name"], special["time"]) : this._addLrc(this.options.lrcClassV1, special);
            if (kname) kname["time"] ? this._addLrc(this.options.lrcClassV1, kname["name"], kname["time"]) : this._addLrc(this.options.lrcClassV1, kname);
            if(lrcs)
                for ( var index in lrcs) {
                    this._addLrc(this.options.lrcClassV1, lrcs[index]["name"], lrcs[index]["time"]);
                }
            if (end) end["time"] ? this._addLrc(this.classV1, end["end"], end["time"]) : this.this._addLrc(this.options.lrcClassV1, end);
        ...
        _getLrcByValue : function(val){
            le = this.options.lrc_lines.length
            val = "^"+val+".*";
            for ( var i = 0; i < le; i++) {
                if (this.lrc_lines[i].lang.search(val) == 0) return this.options.lrc_lines[i];
            }
            return null;
        },
        _getLrcByValueInd : function(val){
            le = this.options.lrc_lines.length - 1
            for ( var i = le; i >= 0; i--) {
                p = this.options.lrc_lines[i].lang;
                if(p)
                    if (p < val) return this.options.lrc_lines[i];
            }
            return null;
        },
        _lrcTorun : function(time){
            time = this._lrctoTimer(Math.round(time * 100) / 100);
            var line = this._getLrcByValueInd(time);
            if(line)
                if(this.upkp!=line){
                    $(line).attr("class","v2");
                    this._lrcUpTop(line);
                    if(this.upkp)$(this.upkp).attr("class","v1");
                    this.upkp = line;
                }
        },
        _lrcUpTop : function(obj){
            var thisTop = obj.offsetTop;   
            var st = $(this.options.lrcid).get(0).scrollTop;
            var nextTop = thisTop-this.options.lrcCenter;  
            this._lrcmyf($(this.options.lrcid).get(0), st, nextTop);
        },
        _lrcmyf:function(obj,f,m){
            if(this.options._lrcTimer!=null){
                clearTimeout(this.options._lrcTimer);
            }
            this._isUpOrDown(obj, f, m);
        },
        _isUpOrDown : function(obj,f,m){
            (f < m) ? this._lrcToUp(obj, f, m) : this._lrcToDown(obj, f, m);
        },
        _lrctoTimer : function( time ) {
            var  m, s;
        ...
        _lrcToUp: function (obj, f, m) {
            var _this = this;
            this.options._lrcTimer = setInterval(function () {
                if(f>=m){
                    clearTimeout(timer);timer=null;obj.scrollTop = m;
                }
	     			
                obj.scrollTop = f;
                f = _this._lrcGetUpValue(f, m);
	     			
            }, 10);
        },
        _lrcToDown: function (obj, f, m) {
            var _this = this;
            this.options._lrcTimer = setInterval(function () {
                if(f<=m){
                    clearTimeout(timer);timer=null;obj.scrollTop = m;
                ...
                obj.scrollTop = f;
                f= _this._lrcGetDowmValue(f,m);
	     			
            }, 30);
        },
        _lrcGetDowmValue : function(f,m){
            if((f-m)>100){
                f-=30;
            }else if((f-m)>50){
                f-=10;
