	...
	$.fn.keyboard = function (opts) {
		...
		var optionFunc = {
				"k_prev": function(){
					...
					if (viewReg1.test(currentView)) {
				        ...
				        if (s != "") {
				            var first_num = 0;
				            var tempIndex = pageIndexArray[pageindex - 1] + increaseLength;//increaseLengthArray[increaseLength];
				...
				"k_next": function(){
					if (viewReg1.test(currentView)) {
				        pageindex = pageindex + 1;
				        if(pageIndexArray.length<pageindex){
				        	pageIndexArray.push(increaseLength);
				        }else{
				        	pageIndexArray[pageindex - 1] = increaseLength;
				        }
		...
		function pressKey(data, obj) {
		    ...
		    if (reg.test(data[0])){
		    	...
		    	data[0].indexOf('prev') == -1 && data[0].indexOf('next') == -1 && (pageindex = 0);
		    	setTimeout(function(){
		...
		*/
		function GetStr(No, s) {
		    ...
		    var sTmp = "", sChi = "";
		    var ableDisplay = "1";
		    var overWidth = 630 + 22;
		    var tempshow = document.getElementById("tempwordsshow");
		    if(tempshow.innerHTML==="" ||parseInt(document.getElementById("tempwordsshow").offsetWidth)===0){
		    	overWidth -=60;
		    }else{
		    	overWidth -= (parseInt(document.getElementById("tempwordsshow").offsetWidth));
		    }
		    
		    var textWidth = overWidth;
		    //increaseLengthArray = new Array();
		    for (var i = 0; i <= parseInt(overWidth / 60)-1; i++) {
		        if (No + i >= CodeList.length - 1) { break; }
		        sTmp = CodeList[No + i];
		        if (sTmp != undefined && sTmp.indexOf(s) >= 0) {
		            sChi = CodeList[No + i];
		            ...
		            textWidth = textWidth -( OutChi[i].length * 25 + 35);
		            if (ableDisplay == "1") {
		    ...
		    //document.getElementById("ChiList").innerHTML += "<span class=\"singleText\">"+ CodeList[LastNo+1].substr(CodeList[LastNo+1].lastIndexOf(SPACECHAR) + 1) +'</span>';
		...
		function keyboardInit(p){
		    ...
		    keyboardstr.push("<div id=\"inputcontainerzoom\">");
